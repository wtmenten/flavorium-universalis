...
|---| `EUV_1.3.x_FU_Rhomania_0.1.0_exarchate_map.png` | The prefectures as live subjects. Best case is two or more of the four seated at once (Ravenna, Carthage, Spania, the Gauls) with Constantinople still the capital, since that is what sells the four-prefecture design. A shot that makes the capital point obvious beats one that does not. |
|---| `EUV_1.3.x_FU_Rhomania_0.1.0_the_west_situation.png` | The **West** situation panel with both counters visible. Ideally caught with imperial grip and western autonomy BOTH high, because the diary makes a point of them not being two ends of one bar and a shot showing them opposed would say the opposite. |
|---| `EUV_1.3.x_FU_Rhomania_0.1.0_consistory_vote.png` | A live vote in the consistory, preferably on **The Rite**, with the vote weights showing. Worth most with three or more prefectures seated, because that is the point at which the emperor's three votes stop being enough and the whole design becomes legible in one frame. Requires the consistory to have been convened. |
...
|---| `EUV_1.3.x_FU_Rhomania_0.1.0_mission_tree.png` | The **Renovatio Imperii** pack zoomed out far enough to show its shape: 86 tasks, five roots, three parallel pillars, four gateways, and the branches reconverging. No node opens more than four things, so it should read as a network with a readable width rather than as a fan. This is the shot that has to sell the tree as a network rather than a list, so frame width over legibility. Needs Mission Packs enabled. Task icons are not yet supplied and will render blank. |
