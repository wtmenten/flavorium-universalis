...
- **Vanilla file overrides (maintenance liability)**: `in_game/common/resolutions/policy_vote.txt` is a **verbatim copy of the vanilla resolution** with one added `else_if` in its `votes` block (weights the `cc_bop_coalition` congress by `country_tax_base + 0.5 × great_power_score`). There is no per-IO hook for vote weight, so the whole resolution must be shadowed. During EA, a Paradox patch to that resolution will be silently masked by this copy — **re-sync it from `game/in_game/common/resolutions/policy_vote.txt` after game updates** and re-apply the marked `cc_bop_coalition` branch. This is the only intentional vanilla-file override in the mod; avoid adding more.
